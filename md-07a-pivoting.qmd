...
1.  Use the `read_csv()` (**Note**: Watch out for the `_` and don't use the `.` as in `read.csv()`) function to import the "[msw-generation-and-composition-by-income.csv](https://github.com/ds4owd-001/md-07-assignments-kelseyshaw.git)" data from the `data` directory and assign it to an object with the name `waste_data`.
